  ...
  return <section className="py-16 md:py-24 px-6 md:px-4 bg-light">
      <div className="container mx-auto max-w-7xl">
          ...
          <div className="relative flex justify-center items-center">
            <div className="relative w-full max-w-md">
              ...
              <div className="relative space-y-3">
                ...
                return <div key={index} className="flex items-center justify-end gap-3">
                      <span className="text-xs md:text-base font-medium text-bg-dark/70 whitespace-pre-line text-right flex-shrink-0">
                        {step.label}
                      </span>
                      <div className={`rounded-lg h-16 md:h-20 flex items-center justify-center shadow-lg ${index % 2 === 0 ? 'bg-bg-dark' : 'bg-white'}`} style={{
                    width: `${widthPercent}%`
                  }}>
                        <span className={`text-2xl md:text-3xl font-bold ${index % 2 === 0 ? 'text-white' : 'text-bg-dark'}`}>
                          {step.number}
                        </span>
                      </div>
                    </div>;
        ...
        <div className="grid md:grid-cols-2 gap-5 md:gap-6 mb-10 md:mb-12">
          {mainSteps.map((step, index) => <div key={index} className={`rounded-2xl p-6 md:p-8 shadow-lg hover:shadow-xl transition-all duration-300 hover:scale-[1.02] ${index % 2 === 0 ? 'bg-bg-dark' : 'bg-white'}`}>
              <div className="flex flex-col md:flex-row items-start gap-4 text-center md:text-left">
                <div className={`flex-shrink-0 w-14 h-14 rounded-xl flex items-center justify-center mx-auto md:mx-0 ${index % 2 === 0 ? 'bg-white' : 'bg-bg-dark'}`}>
                  <span className={`text-2xl font-bold ${index % 2 === 0 ? 'text-bg-dark' : 'text-white'}`}>
                    {step.number}.
                  </span>
                </div>
                <div className="flex-1 space-y-2">
                  <h3 className={`text-lg md:text-xl font-bold ${index % 2 === 0 ? 'text-white' : 'text-bg-dark'}`}>
                    {step.title}
                  </h3>
                  <p className={`leading-relaxed text-sm md:text-base ${index % 2 === 0 ? 'text-white/90' : 'text-bg-dark/80'}`}>
                    {step.description}
                  </p>
                </div>
              </div>
